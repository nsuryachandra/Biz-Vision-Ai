...
BUSINESS_TYPES = {
    ...
    "Food Service / Restaurant": ["restaurant", "cafe", "food truck", "bakery", "kitchen", "pizzeria", "coffee shop"],
...
def extract_location(text):
    ...
    # Look for patterns like "in Hyderabad", "based in New York", "at Mumbai"
    patterns = [
        r"(?:in|at|based in|located in|for|around|near)\s+([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)*)",
    ...
    # Fallback to checking words. Let's see if there's any capitalized word towards the end
    words = text.split()
    for i, word in enumerate(words):
        # If it is capitalized, and not at the very beginning of the sentence
        if i > 0 and word[0].isupper() and word.lower() not in STOP_WORDS:
            # Let's see if we can chain consecutive capitalized words
            location = word
            j = i + 1
...
def extract_keywords(text):
    ...
    clean_text = re.sub(r'\b(?:in|at|based in|located in|for)\s+[A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)*', '', text)
